class DashboardController < ApplicationController
  before_action :authenticate_user!
    ...
    @this_month_daily_records = current_user.daily_records.where(recorded_on: start..finish).select(:id, :recorded_on, :weight, :user_id).order(:recorded_on).includes(:habit_checks, user: :habits)
